...
//					Hibernate再取這兩個主key的值時自己會從被set進去明細檔VO的兩個主檔VO中取出
					...
					//ordDetailVO.setProd_id(request.getParameter("prod_id" + String.valueOf(i)));
					...
					ordDetailVO.setProd_price(
							Double.parseDouble(request.getParameter("prod_price" + String.valueOf(i))));
			...
			return "redirect:/ORDER/SelectOrdAll.jsp";
	...
	public String getOrderDate(ModelMap model,HttpServletRequest request) throws Exception {
		...
		return "/ORDER/SelectOrdAll";
	}
	
	@RequestMapping(method = RequestMethod.POST,value ={"/Querydetail_DeleteOrd.do","/ORDER/Querydetail_DeleteOrd.do"})
